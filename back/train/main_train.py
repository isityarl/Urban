...
def train_parallel_dqn(num_envs=4, episodes=50, pre_model=None):
    ...
    log_dir = "back/res/logs/DQN/details"
    ...
    for episode in range(start_episode, episodes):
        ...
        acc_rewards = [defaultdict(float) for _ in range(num_envs)]
        ...
        while not all(done_flags):
            ...
            for i, (next_state, rewards, done, _) in enumerate(next_list):
                ...
                for tl, act in actions_list[i].items():
                    ...
                    acc_rewards[i][tl] += rewards.get(tl, 0.0)

                    if steps_done % action_interval == 0 or last_actions[i] is None:
                        agent.remember(
                            tl,
                            states_list[i][tl],
                            act,
    ...
    pd.DataFrame(rewards_all_episodes).to_csv("back/res/logs/DQN/model_parallel.csv", index=False)
    print("\nTraining finished. Models and rewards saved.")

if __name__ == "__main__":
    train_parallel_dqn(num_envs=4, episodes=1000, pre_model="back/res/models/DQN/model_parallel_ep300.pth")